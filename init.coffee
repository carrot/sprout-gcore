...
exports.beforeRender = (utils, config) ->
  package_name =
    path.join(config.project_base_dir, config.user_id, config.project_name)
  ...
  _get_go_path()
    .then( (go_path) ->
      ...
      if fs.existsSync(target_path) is on
        throw new Error 'target path already exists: ' + target_path
    )
...
_check_go_installed = ->
  exec('go version')
    .catch (e) ->
      str = """
      Go is either not installed or not added to PATH environment variable.
      """
...
_get_go_path = () ->
  ...
  .then( (streams) ->
    ...
    if go_path.length <= 0
      throw new Error "Go path is either not set or empty"
    return go_path
  )
